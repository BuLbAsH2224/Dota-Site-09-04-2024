...
function createMatchInfo(Match){
    ...
    for(let i = 5; i < 10; i++) { 
        ...
        if(Match.players[i].personaname != null){
            ...
            divForStats.append(deathsImg);
            for(let j = 0;j < 3; j++){
                if(Match.players[i][`backpack_${j}`] != 0){
                let image = document.createElement("img")
...
function createPlayerStatistic(obj){
  //  console.log(obj)
    if(document.querySelector(".PlayerStatisticDiv") != null){
        document.querySelector(".PlayerStatisticDiv").remove();
    }
    
    let el = document.createElement("div");
    el.classList.add("PlayerStatisticDiv")
   
    document.querySelector(".PlayerStatisticPreview").append(el);
    let wins = 0;
    let games = 0;
    for (let item of obj) {
        if(item.games != 0){
